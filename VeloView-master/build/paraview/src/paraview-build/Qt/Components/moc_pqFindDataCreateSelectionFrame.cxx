/****************************************************************************
** Meta object code from reading C++ file 'pqFindDataCreateSelectionFrame.h'
**
** Created by: The Qt Meta Object Compiler version 63 (Qt 4.8.6)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../../paraview/Qt/Components/pqFindDataCreateSelectionFrame.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'pqFindDataCreateSelectionFrame.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.6. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_pqFindDataCreateSelectionFrame[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       5,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       1,       // signalCount

 // signals: signature, parameters, type, tag, flags
      32,   31,   31,   31, 0x05,

 // slots: signature, parameters, type, tag, flags
      48,   31,   31,   31, 0x0a,
      71,   31,   31,   31, 0x08,
      86,   31,   31,   31, 0x08,
      97,   31,   31,   31, 0x08,

       0        // eod
};

static const char qt_meta_stringdata_pqFindDataCreateSelectionFrame[] = {
    "pqFindDataCreateSelectionFrame\0\0"
    "helpRequested()\0setPort(pqOutputPort*)\0"
    "refreshQuery()\0runQuery()\0"
    "onSelectionChanged(pqOutputPort*)\0"
};

void pqFindDataCreateSelectionFrame::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        pqFindDataCreateSelectionFrame *_t = static_cast<pqFindDataCreateSelectionFrame *>(_o);
        switch (_id) {
        case 0: _t->helpRequested(); break;
        case 1: _t->setPort((*reinterpret_cast< pqOutputPort*(*)>(_a[1]))); break;
        case 2: _t->refreshQuery(); break;
        case 3: _t->runQuery(); break;
        case 4: _t->onSelectionChanged((*reinterpret_cast< pqOutputPort*(*)>(_a[1]))); break;
        default: ;
        }
    }
}

const QMetaObjectExtraData pqFindDataCreateSelectionFrame::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject pqFindDataCreateSelectionFrame::staticMetaObject = {
    { &QWidget::staticMetaObject, qt_meta_stringdata_pqFindDataCreateSelectionFrame,
      qt_meta_data_pqFindDataCreateSelectionFrame, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &pqFindDataCreateSelectionFrame::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *pqFindDataCreateSelectionFrame::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *pqFindDataCreateSelectionFrame::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_pqFindDataCreateSelectionFrame))
        return static_cast<void*>(const_cast< pqFindDataCreateSelectionFrame*>(this));
    return QWidget::qt_metacast(_clname);
}

int pqFindDataCreateSelectionFrame::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QWidget::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 5)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 5;
    }
    return _id;
}

// SIGNAL 0
void pqFindDataCreateSelectionFrame::helpRequested()
{
    QMetaObject::activate(this, &staticMetaObject, 0, 0);
}
QT_END_MOC_NAMESPACE
