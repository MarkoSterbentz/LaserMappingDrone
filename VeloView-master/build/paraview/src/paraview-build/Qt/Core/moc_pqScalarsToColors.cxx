/****************************************************************************
** Meta object code from reading C++ file 'pqScalarsToColors.h'
**
** Created by: The Qt Meta Object Compiler version 63 (Qt 4.8.6)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../../paraview/Qt/Core/pqScalarsToColors.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'pqScalarsToColors.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.6. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_pqScalarsToColors[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       5,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       1,       // signalCount

 // signals: signature, parameters, type, tag, flags
      19,   18,   18,   18, 0x05,

 // slots: signature, parameters, type, tag, flags
      44,   18,   18,   18, 0x0a,
      72,   67,   18,   18, 0x0a,
      97,   18,   18,   18, 0x0a,
     105,   18,   18,   18, 0x09,

       0        // eod
};

static const char qt_meta_stringdata_pqScalarsToColors[] = {
    "pqScalarsToColors\0\0componentOrModeChanged()\0"
    "hideUnusedScalarBars()\0lock\0"
    "setScalarRangeLock(bool)\0build()\0"
    "checkRange()\0"
};

void pqScalarsToColors::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        pqScalarsToColors *_t = static_cast<pqScalarsToColors *>(_o);
        switch (_id) {
        case 0: _t->componentOrModeChanged(); break;
        case 1: _t->hideUnusedScalarBars(); break;
        case 2: _t->setScalarRangeLock((*reinterpret_cast< bool(*)>(_a[1]))); break;
        case 3: _t->build(); break;
        case 4: _t->checkRange(); break;
        default: ;
        }
    }
}

const QMetaObjectExtraData pqScalarsToColors::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject pqScalarsToColors::staticMetaObject = {
    { &pqProxy::staticMetaObject, qt_meta_stringdata_pqScalarsToColors,
      qt_meta_data_pqScalarsToColors, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &pqScalarsToColors::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *pqScalarsToColors::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *pqScalarsToColors::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_pqScalarsToColors))
        return static_cast<void*>(const_cast< pqScalarsToColors*>(this));
    return pqProxy::qt_metacast(_clname);
}

int pqScalarsToColors::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = pqProxy::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 5)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 5;
    }
    return _id;
}

// SIGNAL 0
void pqScalarsToColors::componentOrModeChanged()
{
    QMetaObject::activate(this, &staticMetaObject, 0, 0);
}
QT_END_MOC_NAMESPACE
