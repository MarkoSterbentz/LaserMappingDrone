...
#include <vtksys/cstddef>        // size_t
#include <vtksys/stl/algorithm>  // lower_bound
#include <vtksys/stl/functional> // unary_function
#include <vtksys/stl/iterator>   // iterator_traits
#include <vtksys/stl/memory>     // allocator
#include <vtksys/stl/utility>    // pair
#include <vtksys/stl/vector>     // vector
...
#if vtksys_STL_HAS_ALLOCATOR_TEMPLATE
# define vtksys_HASH_DEFAULT_ALLOCATOR(T) vtksys_stl::allocator< T >
#elif vtksys_STL_HAS_ALLOCATOR_NONTEMPLATE
# define vtksys_HASH_DEFAULT_ALLOCATOR(T) vtksys_stl::allocator
#else
# define vtksys_HASH_DEFAULT_ALLOCATOR(T) vtksys_stl::alloc
#endif

#if vtksys_STL_HAS_ALLOCATOR_OBJECTS
# define vtksys_HASH_BUCKETS_INIT(__a) _M_buckets(__a)
# define vtksys_HASH_BUCKETS_GET_ALLOCATOR(__b) , __b.get_allocator()
#else
# define vtksys_HASH_BUCKETS_INIT(__a) _M_buckets()
# define vtksys_HASH_BUCKETS_GET_ALLOCATOR(__b)
#endif
...
#if !vtksys_STL_HAS_ALLOCATOR_REBIND
...
#if vtksys_STL_HAS_ALLOCATOR_MAX_SIZE_ARGUMENT
...
struct _Hashtable_iterator {
  ...
  typedef vtksys_stl::forward_iterator_tag iterator_category;
...
inline size_t _stl_next_prime(size_t __n)
{
  ...
  const unsigned long* pos = vtksys_stl::lower_bound(__first, __last, __n);
...
private:
  ...
  typedef vtksys_stl::vector<_Node*,_M_node_ptr_allocator_type> _M_buckets_type;
#else
public:
...
public:
  ...
  hashtable(const hashtable& __ht)
    : _M_node_allocator(__ht.get_allocator()),
      ...
      vtksys_HASH_BUCKETS_INIT(__ht.get_allocator()),
      _M_num_elements(0)
  {
    _M_copy_from(__ht);
  }
  ...
  {
    vtksys_stl::swap(_M_hash, __ht._M_hash);
    vtksys_stl::swap(_M_equals, __ht._M_equals);
    vtksys_stl::swap(_M_get_key, __ht._M_get_key);
    _M_buckets.swap(__ht._M_buckets);
    vtksys_stl::swap(_M_num_elements, __ht._M_num_elements);
  }
  ...
  friend bool operator==vtksys_CXX_NULL_TEMPLATE_ARGS(const hashtable&,
                                                                 const hashtable&);
...
public:
  ...
  vtksys_stl::pair<iterator, bool> insert_unique(const value_type& __obj)
  {
  ...
  vtksys_stl::pair<iterator, bool> insert_unique_noresize(const value_type& __obj);
  iterator insert_equal_noresize(const value_type& __obj);

#if vtksys_STL_HAS_ITERATOR_TRAITS
# define vtksys_HASH_ITERATOR_CATEGORY(T,I) \
  typename vtksys_stl::iterator_traits< T >::iterator_category()
#elif vtksys_STL_HAS_ITERATOR_CATEGORY
# define vtksys_HASH_ITERATOR_CATEGORY(T,I) \
  vtksys_stl::iterator_category( I )
#elif vtksys_STL_HAS___ITERATOR_CATEGORY
# define vtksys_HASH_ITERATOR_CATEGORY(T,I) \
  vtksys_stl::__iterator_category( I )
#endif

#if vtksys_CXX_HAS_MEMBER_TEMPLATES && defined(vtksys_HASH_ITERATOR_CATEGORY)
  template <class _InputIterator>
  void insert_unique(_InputIterator __f, _InputIterator __l)
  {
    insert_unique(__f, __l,
      vtksys_HASH_ITERATOR_CATEGORY(_InputIterator, __f));
  }
  ...
  void insert_unique(_ForwardIterator __f, _ForwardIterator __l,
                     vtksys_stl::forward_iterator_tag)
  {
    size_type __n = 0;
    vtksys_stl::distance(__f, __l, __n);
  ...
  vtksys_stl::pair<iterator, iterator>
  equal_range(const key_type& __key);

  vtksys_stl::pair<const_iterator, const_iterator>
...
template <class _Val, class _Key, class _HF, class _Ex, class _Eq, class _All>
vtksys_stl::pair<vtksys_CXX_DECL_TYPENAME hashtable<_Val,_Key,_HF,_Ex,_Eq,_All>::iterator, bool>
  ...
  ::insert_unique_noresize(const value_type& __obj)
{
  ...
  for (_Node* __cur = __first; __cur; __cur = __cur->_M_next)
    if (_M_equals(_M_get_key(__cur->_M_val), _M_get_key(__obj)))
      return vtksys_stl::pair<iterator, bool>(iterator(__cur, this), false);

  _Node* __tmp = _M_new_node(__obj);
  __tmp->_M_next = __first;
  _M_buckets[__n] = __tmp;
  ++_M_num_elements;
  return vtksys_stl::pair<iterator, bool>(iterator(__tmp, this), true);
}
...
template <class _Val, class _Key, class _HF, class _Ex, class _Eq, class _All>
vtksys_stl::pair<vtksys_CXX_DECL_TYPENAME hashtable<_Val,_Key,_HF,_Ex,_Eq,_All>::iterator,
     vtksys_CXX_DECL_TYPENAME hashtable<_Val,_Key,_HF,_Ex,_Eq,_All>::iterator>
hashtable<_Val,_Key,_HF,_Ex,_Eq,_All>::equal_range(const key_type& __key)
{
  typedef vtksys_stl::pair<iterator, iterator> _Pii;
  const size_type __n = _M_bkt_num_key(__key);
...
template <class _Val, class _Key, class _HF, class _Ex, class _Eq, class _All>
vtksys_stl::pair<vtksys_CXX_DECL_TYPENAME hashtable<_Val,_Key,_HF,_Ex,_Eq,_All>::const_iterator,
     vtksys_CXX_DECL_TYPENAME hashtable<_Val,_Key,_HF,_Ex,_Eq,_All>::const_iterator>
hashtable<_Val,_Key,_HF,_Ex,_Eq,_All>
  ::equal_range(const key_type& __key) const
{
  typedef vtksys_stl::pair<const_iterator, const_iterator> _Pii;
  const size_type __n = _M_bkt_num_key(__key);
  ...
  if (__num_elements_hint > __old_n) {
    ...
    if (__n > __old_n) {
      _M_buckets_type __tmp(
        __n, (_Node*)(0)
        vtksys_HASH_BUCKETS_GET_ALLOCATOR(_M_buckets));
      try {
...
// Normally the comparison operators should be found in the vtksys
...
#if !vtksys_CXX_HAS_ARGUMENT_DEPENDENT_LOOKUP
using vtksys::operator==;
using vtksys::operator!=;
#endif
