// ClientServer wrapper for vtkCutter object
//
...
vtkObjectBase *vtkCutterClientServerNewCommand(void* /*ctx*/)
{
  return vtkCutter::New();
}


int VTK_EXPORT vtkCutterCommand(vtkClientServerInterpreter *arlu, vtkObjectBase *ob, const char *method, const vtkClientServerStream& msg, vtkClientServerStream& resultStream, void* /*ctx*/)
{
  vtkCutter *op = vtkCutter::SafeDownCast(ob);
  if(!op)
    {
    vtkOStrStreamWrapper vtkmsg;
    vtkmsg << "Cannot cast " << ob->GetClassName() << " object to vtkCutter.  "
  ...
  if (!strcmp("GetCellTypeDimensions",method) && msg.GetNumberOfArguments(0) == 3)
    {
    vtkClientServerStreamDataArg<unsigned char   > temp0(msg, 0, 2);
    if(temp0)
      {
      op->GetCellTypeDimensions(temp0);
      return 1;
      }
    }
  ...
  vtkOStrStreamWrapper vtkmsg;
  vtkmsg << "Object type: vtkCutter, could not find requested method: \""
...
//-------------------------------------------------------------------------auto
void VTK_EXPORT vtkCutter_Init(vtkClientServerInterpreter* csi)
{
  static vtkClientServerInterpreter* last = NULL;
  if(last != csi)
    {
    last = csi;
    csi->AddNewInstanceFunction("vtkCutter", vtkCutterClientServerNewCommand);
    csi->AddCommandFunction("vtkCutter", vtkCutterCommand);
    }
}
