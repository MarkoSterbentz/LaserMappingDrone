// ClientServer wrapper for vtkPolyDataAlgorithm object
//
...
vtkObjectBase *vtkPolyDataAlgorithmClientServerNewCommand(void* /*ctx*/)
{
  return vtkPolyDataAlgorithm::New();
}


int VTK_EXPORT vtkPolyDataAlgorithmCommand(vtkClientServerInterpreter *arlu, vtkObjectBase *ob, const char *method, const vtkClientServerStream& msg, vtkClientServerStream& resultStream, void* /*ctx*/)
{
  vtkPolyDataAlgorithm *op = vtkPolyDataAlgorithm::SafeDownCast(ob);
  if(!op)
    {
    vtkOStrStreamWrapper vtkmsg;
    vtkmsg << "Cannot cast " << ob->GetClassName() << " object to vtkPolyDataAlgorithm.  "
  ...
  if (!strcmp("GetPolyDataInput",method) && msg.GetNumberOfArguments(0) == 3)
    {
    int      temp0;
    vtkPolyData  *temp20;
    if(msg.GetArgument(0, 2, &temp0))
      {
      temp20 = (op)->GetPolyDataInput(temp0);
      resultStream.Reset();
      resultStream << vtkClientServerStream::Reply << (vtkObjectBase *)temp20 << vtkClientServerStream::End;
      return 1;
      }
  ...
  vtkOStrStreamWrapper vtkmsg;
  vtkmsg << "Object type: vtkPolyDataAlgorithm, could not find requested method: \""
...
//-------------------------------------------------------------------------auto
void VTK_EXPORT vtkPolyDataAlgorithm_Init(vtkClientServerInterpreter* csi)
{
  static vtkClientServerInterpreter* last = NULL;
  if(last != csi)
    {
    last = csi;
    csi->AddNewInstanceFunction("vtkPolyDataAlgorithm", vtkPolyDataAlgorithmClientServerNewCommand);
    csi->AddCommandFunction("vtkPolyDataAlgorithm", vtkPolyDataAlgorithmCommand);
    }
}
