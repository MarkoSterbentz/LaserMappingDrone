// ClientServer wrapper for vtkPlotParallelCoordinates object
//
...
vtkObjectBase *vtkPlotParallelCoordinatesClientServerNewCommand(void* /*ctx*/)
{
  return vtkPlotParallelCoordinates::New();
}


int VTK_EXPORT vtkPlotParallelCoordinatesCommand(vtkClientServerInterpreter *arlu, vtkObjectBase *ob, const char *method, const vtkClientServerStream& msg, vtkClientServerStream& resultStream, void* /*ctx*/)
{
  vtkPlotParallelCoordinates *op = vtkPlotParallelCoordinates::SafeDownCast(ob);
  if(!op)
    {
    vtkOStrStreamWrapper vtkmsg;
    vtkmsg << "Cannot cast " << ob->GetClassName() << " object to vtkPlotParallelCoordinates.  "
  ...
  if (!strcmp("NewInstance",method) && msg.GetNumberOfArguments(0) == 2)
    {
    vtkPlotParallelCoordinates  *temp20;
      {
  ...
  if (!strcmp("SetSelectionRange",method) && msg.GetNumberOfArguments(0) == 5)
    {
    int      temp0;
    float    temp1;
    float    temp2;
    ...
    if(msg.GetArgument(0, 2, &temp0) &&
      ...
      {
      temp20 = (op)->SetSelectionRange(temp0,temp1,temp2);
      resultStream.Reset();
      resultStream << vtkClientServerStream::Reply << temp20 << vtkClientServerStream::End;
      return 1;
      }
    }
  if (!strcmp("ResetSelectionRange",method) && msg.GetNumberOfArguments(0) == 2)
    {
    bool   temp20;
      {
      temp20 = (op)->ResetSelectionRange();
      resultStream.Reset();
      resultStream << vtkClientServerStream::Reply << temp20 << vtkClientServerStream::End;
      return 1;
      }
  ...
  vtkOStrStreamWrapper vtkmsg;
  vtkmsg << "Object type: vtkPlotParallelCoordinates, could not find requested method: \""
...
//-------------------------------------------------------------------------auto
void VTK_EXPORT vtkPlotParallelCoordinates_Init(vtkClientServerInterpreter* csi)
{
  static vtkClientServerInterpreter* last = NULL;
  if(last != csi)
    {
    last = csi;
    csi->AddNewInstanceFunction("vtkPlotParallelCoordinates", vtkPlotParallelCoordinatesClientServerNewCommand);
    csi->AddCommandFunction("vtkPlotParallelCoordinates", vtkPlotParallelCoordinatesCommand);
    }
}
