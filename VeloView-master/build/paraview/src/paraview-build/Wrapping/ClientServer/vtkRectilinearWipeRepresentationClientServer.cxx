// ClientServer wrapper for vtkRectilinearWipeRepresentation object
//
...
vtkObjectBase *vtkRectilinearWipeRepresentationClientServerNewCommand(void* /*ctx*/)
{
  return vtkRectilinearWipeRepresentation::New();
}


int VTK_EXPORT vtkRectilinearWipeRepresentationCommand(vtkClientServerInterpreter *arlu, vtkObjectBase *ob, const char *method, const vtkClientServerStream& msg, vtkClientServerStream& resultStream, void* /*ctx*/)
{
  vtkRectilinearWipeRepresentation *op = vtkRectilinearWipeRepresentation::SafeDownCast(ob);
  if(!op)
    {
    vtkOStrStreamWrapper vtkmsg;
    vtkmsg << "Cannot cast " << ob->GetClassName() << " object to vtkRectilinearWipeRepresentation.  "
  ...
  if (!strcmp("SetRectilinearWipe",method) && msg.GetNumberOfArguments(0) == 3)
    {
    vtkImageRectilinearWipe  *temp0;
    if(vtkClientServerStreamGetArgumentObject(msg, 0, 2, &temp0, "vtkImageRectilinearWipe"))
      {
      op->SetRectilinearWipe(temp0);
      return 1;
      }
    }
  if (!strcmp("GetRectilinearWipe",method) && msg.GetNumberOfArguments(0) == 2)
    {
    vtkImageRectilinearWipe  *temp20;
      {
      temp20 = (op)->GetRectilinearWipe();
      resultStream.Reset();
      resultStream << vtkClientServerStream::Reply << (vtkObjectBase *)temp20 << vtkClientServerStream::End;
      return 1;
      }
  ...
  vtkOStrStreamWrapper vtkmsg;
  vtkmsg << "Object type: vtkRectilinearWipeRepresentation, could not find requested method: \""
...
//-------------------------------------------------------------------------auto
void VTK_EXPORT vtkRectilinearWipeRepresentation_Init(vtkClientServerInterpreter* csi)
{
  static vtkClientServerInterpreter* last = NULL;
  if(last != csi)
    {
    last = csi;
    csi->AddNewInstanceFunction("vtkRectilinearWipeRepresentation", vtkRectilinearWipeRepresentationClientServerNewCommand);
    csi->AddCommandFunction("vtkRectilinearWipeRepresentation", vtkRectilinearWipeRepresentationCommand);
    }
}
