// ClientServer wrapper for vtkExtractGeometry object
//
...
vtkObjectBase *vtkExtractGeometryClientServerNewCommand(void* /*ctx*/)
{
  return vtkExtractGeometry::New();
}


int VTK_EXPORT vtkExtractGeometryCommand(vtkClientServerInterpreter *arlu, vtkObjectBase *ob, const char *method, const vtkClientServerStream& msg, vtkClientServerStream& resultStream, void* /*ctx*/)
{
  vtkExtractGeometry *op = vtkExtractGeometry::SafeDownCast(ob);
  if(!op)
    {
    vtkOStrStreamWrapper vtkmsg;
    vtkmsg << "Cannot cast " << ob->GetClassName() << " object to vtkExtractGeometry.  "
  ...
  if (!strcmp("SetExtractOnlyBoundaryCells",method) && msg.GetNumberOfArguments(0) == 3)
    {
    int      temp0;
    if(msg.GetArgument(0, 2, &temp0))
      {
  ...
  if (!strcmp("GetExtractOnlyBoundaryCells",method) && msg.GetNumberOfArguments(0) == 2)
    {
    int      temp20;
      {
      temp20 = (op)->GetExtractOnlyBoundaryCells();
      resultStream.Reset();
      resultStream << vtkClientServerStream::Reply << temp20 << vtkClientServerStream::End;
      return 1;
      }
    }
  if (!strcmp("ExtractOnlyBoundaryCellsOn",method) && msg.GetNumberOfArguments(0) == 2)
    {
      {
  ...
  if (!strcmp("ExtractOnlyBoundaryCellsOff",method) && msg.GetNumberOfArguments(0) == 2)
    {
      {
  ...
  vtkOStrStreamWrapper vtkmsg;
  vtkmsg << "Object type: vtkExtractGeometry, could not find requested method: \""
...
//-------------------------------------------------------------------------auto
void VTK_EXPORT vtkExtractGeometry_Init(vtkClientServerInterpreter* csi)
{
  static vtkClientServerInterpreter* last = NULL;
  if(last != csi)
    {
    last = csi;
    csi->AddNewInstanceFunction("vtkExtractGeometry", vtkExtractGeometryClientServerNewCommand);
    csi->AddCommandFunction("vtkExtractGeometry", vtkExtractGeometryCommand);
    }
}
