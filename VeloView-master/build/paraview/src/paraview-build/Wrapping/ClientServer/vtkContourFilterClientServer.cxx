// ClientServer wrapper for vtkContourFilter object
//
...
vtkObjectBase *vtkContourFilterClientServerNewCommand(void* /*ctx*/)
{
  return vtkContourFilter::New();
}


int VTK_EXPORT vtkContourFilterCommand(vtkClientServerInterpreter *arlu, vtkObjectBase *ob, const char *method, const vtkClientServerStream& msg, vtkClientServerStream& resultStream, void* /*ctx*/)
{
  vtkContourFilter *op = vtkContourFilter::SafeDownCast(ob);
  if(!op)
    {
    vtkOStrStreamWrapper vtkmsg;
    vtkmsg << "Cannot cast " << ob->GetClassName() << " object to vtkContourFilter.  "
  ...
  if (!strcmp("SetUseScalarTree",method) && msg.GetNumberOfArguments(0) == 3)
    {
    int      temp0;
    if(msg.GetArgument(0, 2, &temp0))
      {
  ...
  if (!strcmp("GetUseScalarTree",method) && msg.GetNumberOfArguments(0) == 2)
    {
    int      temp20;
      {
      temp20 = (op)->GetUseScalarTree();
      resultStream.Reset();
      resultStream << vtkClientServerStream::Reply << temp20 << vtkClientServerStream::End;
      return 1;
      }
    }
  if (!strcmp("UseScalarTreeOn",method) && msg.GetNumberOfArguments(0) == 2)
    {
      {
      op->UseScalarTreeOn();
      return 1;
  ...
  if (!strcmp("UseScalarTreeOff",method) && msg.GetNumberOfArguments(0) == 2)
    {
      {
      op->UseScalarTreeOff();
      return 1;
  ...
  if (!strcmp("SetScalarTree",method) && msg.GetNumberOfArguments(0) == 3)
    {
    vtkScalarTree  *temp0;
    if(vtkClientServerStreamGetArgumentObject(msg, 0, 2, &temp0, "vtkScalarTree"))
      {
      op->SetScalarTree(temp0);
      return 1;
      }
    }
  if (!strcmp("GetScalarTree",method) && msg.GetNumberOfArguments(0) == 2)
    {
    vtkScalarTree  *temp20;
      {
      temp20 = (op)->GetScalarTree();
      resultStream.Reset();
      resultStream << vtkClientServerStream::Reply << (vtkObjectBase *)temp20 << vtkClientServerStream::End;
      return 1;
      }
  ...
  vtkOStrStreamWrapper vtkmsg;
  vtkmsg << "Object type: vtkContourFilter, could not find requested method: \""
...
//-------------------------------------------------------------------------auto
void VTK_EXPORT vtkContourFilter_Init(vtkClientServerInterpreter* csi)
{
  static vtkClientServerInterpreter* last = NULL;
  if(last != csi)
    {
    last = csi;
    csi->AddNewInstanceFunction("vtkContourFilter", vtkContourFilterClientServerNewCommand);
    csi->AddCommandFunction("vtkContourFilter", vtkContourFilterCommand);
    }
}
