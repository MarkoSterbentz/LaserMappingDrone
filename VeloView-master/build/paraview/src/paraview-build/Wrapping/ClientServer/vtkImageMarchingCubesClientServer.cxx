// ClientServer wrapper for vtkImageMarchingCubes object
//
...
vtkObjectBase *vtkImageMarchingCubesClientServerNewCommand(void* /*ctx*/)
{
  return vtkImageMarchingCubes::New();
}


int VTK_EXPORT vtkImageMarchingCubesCommand(vtkClientServerInterpreter *arlu, vtkObjectBase *ob, const char *method, const vtkClientServerStream& msg, vtkClientServerStream& resultStream, void* /*ctx*/)
{
  vtkImageMarchingCubes *op = vtkImageMarchingCubes::SafeDownCast(ob);
  if(!op)
    {
    vtkOStrStreamWrapper vtkmsg;
    vtkmsg << "Cannot cast " << ob->GetClassName() << " object to vtkImageMarchingCubes.  "
  ...
  if (!strcmp("GetLocatorPoint",method) && msg.GetNumberOfArguments(0) == 5)
    {
    int      temp0;
    int      temp1;
    int      temp2;
    ...
    if(msg.GetArgument(0, 2, &temp0) &&
      ...
      msg.GetArgument(0, 4, &temp2))
      {
      temp20 = (op)->GetLocatorPoint(temp0,temp1,temp2);
      resultStream.Reset();
      resultStream << vtkClientServerStream::Reply << temp20 << vtkClientServerStream::End;
      return 1;
      }
    }
  if (!strcmp("AddLocatorPoint",method) && msg.GetNumberOfArguments(0) == 6)
    {
    int      temp0;
    int      temp1;
    int      temp2;
  ...
  if (!strcmp("IncrementLocatorZ",method) && msg.GetNumberOfArguments(0) == 2)
    {
      {
  ...
  if (!strcmp("GetInputMemoryLimit",method) && msg.GetNumberOfArguments(0) == 2)
    {
    vtkIdType   temp20;
      {
      temp20 = (op)->GetInputMemoryLimit();
      resultStream.Reset();
      resultStream << vtkClientServerStream::Reply << temp20 << vtkClientServerStream::End;
      return 1;
      }
  ...
  vtkOStrStreamWrapper vtkmsg;
  vtkmsg << "Object type: vtkImageMarchingCubes, could not find requested method: \""
...
//-------------------------------------------------------------------------auto
void VTK_EXPORT vtkImageMarchingCubes_Init(vtkClientServerInterpreter* csi)
{
  static vtkClientServerInterpreter* last = NULL;
  if(last != csi)
    {
    last = csi;
    csi->AddNewInstanceFunction("vtkImageMarchingCubes", vtkImageMarchingCubesClientServerNewCommand);
    csi->AddCommandFunction("vtkImageMarchingCubes", vtkImageMarchingCubesCommand);
    }
}
