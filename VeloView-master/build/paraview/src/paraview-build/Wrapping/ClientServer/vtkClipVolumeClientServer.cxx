// ClientServer wrapper for vtkClipVolume object
//
...
vtkObjectBase *vtkClipVolumeClientServerNewCommand(void* /*ctx*/)
{
  return vtkClipVolume::New();
}


int VTK_EXPORT vtkClipVolumeCommand(vtkClientServerInterpreter *arlu, vtkObjectBase *ob, const char *method, const vtkClientServerStream& msg, vtkClientServerStream& resultStream, void* /*ctx*/)
{
  vtkClipVolume *op = vtkClipVolume::SafeDownCast(ob);
  if(!op)
    {
    vtkOStrStreamWrapper vtkmsg;
    vtkmsg << "Cannot cast " << ob->GetClassName() << " object to vtkClipVolume.  "
  ...
  if (!strcmp("SetMixed3DCellGeneration",method) && msg.GetNumberOfArguments(0) == 3)
    {
    int      temp0;
    if(msg.GetArgument(0, 2, &temp0))
      {
  ...
  if (!strcmp("GetMixed3DCellGeneration",method) && msg.GetNumberOfArguments(0) == 2)
    {
    int      temp20;
      {
      temp20 = (op)->GetMixed3DCellGeneration();
      resultStream.Reset();
      resultStream << vtkClientServerStream::Reply << temp20 << vtkClientServerStream::End;
      return 1;
      }
    }
  if (!strcmp("Mixed3DCellGenerationOn",method) && msg.GetNumberOfArguments(0) == 2)
    {
      {
  ...
  if (!strcmp("Mixed3DCellGenerationOff",method) && msg.GetNumberOfArguments(0) == 2)
    {
      {
  ...
  vtkOStrStreamWrapper vtkmsg;
  vtkmsg << "Object type: vtkClipVolume, could not find requested method: \""
...
//-------------------------------------------------------------------------auto
void VTK_EXPORT vtkClipVolume_Init(vtkClientServerInterpreter* csi)
{
  static vtkClientServerInterpreter* last = NULL;
  if(last != csi)
    {
    last = csi;
    csi->AddNewInstanceFunction("vtkClipVolume", vtkClipVolumeClientServerNewCommand);
    csi->AddCommandFunction("vtkClipVolume", vtkClipVolumeCommand);
    }
}
