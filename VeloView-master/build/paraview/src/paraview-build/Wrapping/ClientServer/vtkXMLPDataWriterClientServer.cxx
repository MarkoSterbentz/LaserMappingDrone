// ClientServer wrapper for vtkXMLPDataWriter object
//
...
int VTK_EXPORT vtkXMLPDataWriterCommand(vtkClientServerInterpreter *arlu, vtkObjectBase *ob, const char *method, const vtkClientServerStream& msg, vtkClientServerStream& resultStream, void* /*ctx*/)
{
  vtkXMLPDataWriter *op = vtkXMLPDataWriter::SafeDownCast(ob);
  if(!op)
    {
    vtkOStrStreamWrapper vtkmsg;
    vtkmsg << "Cannot cast " << ob->GetClassName() << " object to vtkXMLPDataWriter.  "
  ...
  if (!strcmp("SetWriteSummaryFile",method) && msg.GetNumberOfArguments(0) == 3)
    {
    int      temp0;
    if(msg.GetArgument(0, 2, &temp0))
      {
  ...
  if (!strcmp("GetWriteSummaryFile",method) && msg.GetNumberOfArguments(0) == 2)
    {
    int      temp20;
      {
      temp20 = (op)->GetWriteSummaryFile();
      resultStream.Reset();
      resultStream << vtkClientServerStream::Reply << temp20 << vtkClientServerStream::End;
      return 1;
      }
    }
  if (!strcmp("WriteSummaryFileOn",method) && msg.GetNumberOfArguments(0) == 2)
    {
      {
  ...
  if (!strcmp("WriteSummaryFileOff",method) && msg.GetNumberOfArguments(0) == 2)
    {
      {
  ...
  vtkOStrStreamWrapper vtkmsg;
  vtkmsg << "Object type: vtkXMLPDataWriter, could not find requested method: \""
...
//-------------------------------------------------------------------------auto
void VTK_EXPORT vtkXMLPDataWriter_Init(vtkClientServerInterpreter* csi)
{
  static vtkClientServerInterpreter* last = NULL;
  if(last != csi)
    {
    last = csi;
    csi->AddCommandFunction("vtkXMLPDataWriter", vtkXMLPDataWriterCommand);
    }
}
